...
3. Какой порядковый номер у SYN TCP-сегмента, который используется для установления
   ...
   - Sequence Number: 0 (relative sequence number), Sequence Number (raw): 524955332
   - Syn flag =1
...
4. Какой порядковый номер у SYNACK-сегмента, отправленного сервером gaia.cs.umass.edu
   ...
   - Sequence Number: 0    (relative sequence number), Sequence Number (raw): 3470804605
   - Acknowledgment Number: 1    (relative ack number), Acknowledgment number (raw): 524955333
   -  Seq. Number из пункта выше + 1
...
5. Какой порядковый номер у TCP-сегмента, содержащего команду POST протокола HTTP?
   ...
   - Sequence Number: 1    (relative sequence number), Sequence Number (raw): 524955333
   ![Alt text](0.png "Optional title")
6. Рассмотрите TCP-сегмент, содержащий команду POST протокола HTTP, как первый TCP-сегмент 
   ...
   Надо из time ack пакета вычети time post пакета
   - 629	   15.466110	15.610343	0.1560
   - 3549	15.466110	15.612820	0.1585
   - 5009	15.466110	15.612820	0.1585
   - 6469	15.466110	15.612820	0.1585
7. Чему равна пропускная способность (количество байтов, передаваемых в единицу
   ...
   - Windowsize пакетов поделим на RTT 
   - Ответ: 65280 / 0.1567 ≈ 411592 байт/с
