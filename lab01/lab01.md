...
2. Проверьте содержимое ответа сервера. Вернул ли сервер содержимое файла явно? Как вы
   это можете увидеть?
   - Да.
   - В Line-based text data: text/html (10 lines). Content-Type: text/html
3. Теперь проверьте содержимое второго HTTP-запроса GET (из вашего браузера на сторону
   ...
   - Да. 28 Oct 2025 05:59:01 - время актуальной версии файла
...
![Alt text](2_1.png "Optional title"). 
![Alt text](2_2.png "Optional title"). 
...
4. Есть ли в передаваемых данных какая-либо информация заголовка HTTP, связанная с
   сегментацией TCP?
   - Да. [4 Reassembled TCP Segments (4864 bytes): #218(1460), #219(1460), #221(1460), #222(484)]
...
1. Сколько HTTP GET запросов было отправлено вашим браузером? На какие интернет-адреса были отправлены эти GET-запросы?
   - 3 get запроса
   -   1)http://gaia.cs.umass.edu//wireshark-labs/HTTP-wireshark-file4.html http://gaia.cs.umass.edu//pearson.png 3)https://kurose.cslash.net/8E_cover_small.jpg 
...
2. Можете ли вы сказать, загрузил ли ваш браузер два изображения последовательно или
   ...
   - Получилось, что параллельно. Еще не загрузился pearson.jpg а запрос get на вторую картинку уже отправился. 
...
2. Когда ваш браузер отправляет сообщение HTTP GET во второй раз, какое новое поле включается в сообщение HTTP GET?
   - Поле Authorization с login и пароль
